...
public class FollowPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        if(player.transform.position != this.transform.position)
        {
            ...
            if (Mathf.Abs(this.transform.position.y - player.transform.position.y) > 0.5f)
            {
                float distance = this.transform.position.y - player.transform.position.y;
                this.transform.position = new Vector3(this.transform.position.x, this.transform.position.y - (0.2f * distance), this.transform.position.z);
            }
        }
    }
}
